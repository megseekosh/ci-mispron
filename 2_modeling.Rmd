...
library('beepr') # to tell when models are finished
library('parallel') # to tell # of cores present
...
m <- gamtabs(groupcond_model,
        ...
        pnames = c("Intercept (Cochlear Implant: Mispronunciation)", "Typical Hearing", "Typical Hearing: Correct", "Cochlear Implant: Correct"), # the parametric terms
...
interact_model <-  get_gam_predictions(interact_model_nd, # get the dataframe with fit and s.e. of fit
                      ...
                      Condition = recode(Condition, "MP"="Mispronunciation", "real"="Correct \n pronunciation")) %>%
...
```{r, real-mp-facet-plot, fig.cap="Difference smooths (GAMM predictions) by hearing status for correct pronunciations and mispronunciations. The pink smooth represents the point when the smooth for children with TH differs from children with CIs (i.e., reliable effect of group): there is an effect of group upon mispronunciations (L plot: see also difference between turquoise lines in Figure 2), but not correct pronunciations  (R plot: see also difference between yellow lines in Figure 2). Shaded ribbons represent 95% confidence intervals."}
...
```{r, how many unique children w CIs completed both assessments?}
looks %>% 
  distinct(ResearchID, .keep_all = T) %>%
  filter(GFTA_Standard!='NA' & EVT_GSV!='NA') %>% 
  filter(Group=='CochlearImplant') %>%
  NROW()
```

```{r, how many trials for the 33 ci and 24 th who completed both assessments?}
looks %>% 
  filter(GFTA_Standard!='NA' & EVT_GSV!='NA') %>% 
  distinct_at(., vars(ChildStudyID,TargetWord), .keep_all = T) %>%
  count(Group)
```
...
```{r, modeling real-MP differences for CIs, eval=T}
...
get_smooths_difference(gfta_model_CI, 
  ...
  annotate(geom="text", x=1720, y=1.7, size=5, label="Best") +
  annotate(geom="text", x=1720, y=1.27, size=5, label="Better", color='gray35') +
  annotate(geom="text", x=1720, y=1.03, size=5, label="Poorer", color='gray50') +
  theme(legend.position = "none",
        axis.text.x = element_text(face="bold", size=9),
        axis.title.x=element_text(face="bold", size=12),
        axis.text.y = element_text(face="bold", size=9),
...
```{r, CI-raw-plot, fig.cap="Raw response trajectories for proportion of looks to familiar object for children with CIs, by word condition and standardized articulation score. Children were divided into tertiles by score: poorer (median score=57), better (72), and best (96) articulation scores."}
...
ci_kids_plot %>%
  ...
  theme(axis.text = element_text(face="bold",size=8),
      ...
      annotate(geom="text", x=525, y=1, size=4, label="Correct",color='darkgoldenrod3') +
      annotate(geom="text", x=900, y=0.05, size=4, label="Mispronunciation",color='turquoise4')
...
act_stats <- all_cis2 %>%
  summarize(avg_activation = mean(age_at_activation),
...
gamtabs(diff_model,
        ...
        pnames = c("Intercept (Cochlear Implant: Mispronunciation)", "Typical Hearing"), # the parametric terms
        snames = c("s(Time)",
                   "s(Time,Cochlear Implant)",
                   "s(Time,Typical Hearing)",
                   "s(Time,Cochlear Implant Correct - Cochlear Implant Mispronunciation)",
...
gamtabs(gfta_model_CI,
        ...
        pnames = c("Intercept (Mispronunciation)", "Correct"), # the parametric terms
        snames = c("s(Time)",
                   "s(Time,Correct)",
                   "s(Age)",
                   "te(Time,Age; Correct)",
