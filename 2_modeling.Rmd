...
```{r, read in data}
all_kids <- read.csv('./ha_matched_kids.csv') %>% # contains the N=38 kids who are hearing age-matched
...
AIC(condition_base,interact_model) # interact model has much lower AIC
...
saveRDS(interact_model_freml, "../hearingage_matched_models/interact-model-freml.Rdata")
...
hearing_age <- read.csv('../participant_info/hearing_age.csv') %>% distinct(ChildStudyID, .keep_all = T)
